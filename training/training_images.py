...
from models.feature_extraction import DinoV2, SelfAttention
...
class Training:
    def __init__(self, device, config):
        ...
        if self.sequence_length > 1:
            self.attention_model = SelfAttention(self.image_output_size, self.attention_output_size,
                                                 self.device).to(self.device)
            decoder_input_size = self.attention_output_size
        else:
            self.attention_model = None
            decoder_input_size = self.image_output_size

        self.decoder = self.get_homeomorphism_model(decoder_input_size).to(self.device)

        # optimizer and weights
        params = []
        models = [self.decoder, self.attention_model, self.image_encoder]
    ...
    def train(self, dataloader, epoch):
        ...
        for i, item in enumerate(dataloader):
            ...
            if self.sequence_length > 1:
                features = features.view(self.sequence_length, batch_size, self.image_output_size)
        ...
        with torch.no_grad():
            for i, item in enumerate(dataloader):
                ...
                point_face_loss, chamfer_loss_roi, predicted_meshes, predicted_roi_meshes, target_roi_meshes, ee_pos = self.loss(
                    ...
                    True)
                print(point_face_loss, chamfer_loss_roi)
                total_roi_chamfer_loss += chamfer_loss_roi
                total_point_face_loss += point_face_loss
    ...
    def save_models(self):
        ...
        models = [self.decoder, self.attention_model, self.image_encoder]
        names = ["decoder", "attention_model", "image_encoder"]
        for name, model in zip(names, models):
            if model is not None:
                torch.save(model, f"{path}/{name}.pth")


def images_main(config):
