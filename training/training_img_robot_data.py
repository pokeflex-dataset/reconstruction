...
from models.feature_extraction import DinoV2, CrossAttention, ForceFeatures
from pytorch3d.loss import (
    chamfer_distance, point_mesh_face_distance
)
from pytorch3d.io import save_obj
from dataset.dataset_images import ImagesDataset, collate_fn
...
class Training:
    def __init__(self, device, config):
        ...
        self.image_output_size = config['image_feat_output_size']
        self.force_output_size = config['force_feat_output_size']
        self.sequence_length = config['sequence_length']

        # models
        self.image_encoder = DinoV2(1, self.image_output_size).to(self.device)
        self.force_encoder = ForceFeatures(self.force_output_size).to(self.device)
        self.attention_model = CrossAttention(self.image_output_size, self.force_output_size,
                                              self.attention_output_size, self.device).to(self.device)
        ...
        params = []
        models = [self.decoder, self.attention_model, self.image_encoder, self.force_encoder]
    ...
    def load_data(self, data_paths, config):
        ...
        for data_path in data_paths:
            ...
            for i, directory in enumerate(os.listdir(data_path)):
                current_set = ImagesDataset(os.path.join(data_path, directory), config['cameras'], self.sequence_length)
    ...
    def train(self, dataloader, epoch):
        ...
        for i, item in enumerate(dataloader):
            ...
            # get data
            target_meshes, template_vertices, template_faces, images, forces, centers, scales, frames, num_points, names = item
            batch_size = len(frames)

            # get features
            image_features = self.image_encoder.forward(images)
            force_features = self.force_encoder.forward(forces)
            image_features = image_features.view(self.sequence_length, batch_size, self.image_output_size)
            force_features = force_features.view(self.sequence_length, batch_size, self.force_output_size)
            features = self.attention_model.forward(image_features, force_features)
    ...
    def save_models(self):
        ...
        models = [self.decoder, self.attention_model, self.image_encoder, self.force_encoder]
        names = ["decoder", "attention_model", "image_encoder", "force_encoder"]
        for name, model in zip(names, models):
            if model is not None:
                torch.save(model, f"{path}/{name}.pth")


def img_robot_data_main(config):
