...
from models.feature_extraction import SelfAttention
from models.autoencoder import AutoEncoder
from pytorch3d.loss import (
    chamfer_distance, point_mesh_face_distance
)
from pytorch3d.io import save_obj
from dataset.dataset_pointclouds import PointcloudsDataset, collate_fn
...
class Training:
    def __init__(self, device, config):
        ...
        self.pointcloud_output_size = config['pointcloud_feat_output_size']
        self.sequence_length = config['sequence_length']

        # models
        self.pointcloud_encoder = AutoEncoder(self.pointcloud_output_size).to(self.device)
        if self.sequence_length > 1:
            self.attention_model = SelfAttention(self.pointcloud_output_size, self.attention_output_size,
                                                 self.device).to(self.device)
            decoder_input_size = self.attention_output_size
        else:
            self.attention_model = None
            decoder_input_size = self.pointcloud_output_size

        self.decoder = self.get_homeomorphism_model(decoder_input_size).to(self.device)

        # optimizer and weights
        params = []
        models = [self.decoder, self.attention_model, self.pointcloud_encoder]
    ...
    def load_data(self, data_paths, config):
        ...
        for data_path in data_paths:
            ...
            for i, directory in enumerate(os.listdir(data_path)):
                current_set = PointcloudsDataset(os.path.join(data_path, directory), config['pointcloud_source'], self.sequence_length)
    ...
    def train(self, dataloader, epoch):
        ...
        for i, item in enumerate(dataloader):
            ...
            # get data
            target_meshes, template_vertices, template_faces, point_clouds, forces, centers, scales, frames, num_points, names = item
            batch_size = len(frames)

            # get features
            features = self.pointcloud_encoder.encoder.forward(point_clouds.permute(0, 2, 1))
            if self.sequence_length > 1:
                features = features.view(self.sequence_length, batch_size, self.pointcloud_output_size)
    ...
    def save_models(self):
        ...
        models = [self.decoder, self.attention_model, self.pointcloud_encoder]
        names = ["decoder", "attention_model", "pointcloud_encoder"]
        for name, model in zip(names, models):
            if model is not None:
                torch.save(model, f"{path}/{name}.pth")


def pointclouds_main(config):
